            ...
            with open(file_path, 'r') as file:
                ...
                for row in reader:
                    if row and not row[0].startswith('#'):  # 跳过注释行
                        pdb_code = row[0]
                        log_kd_ki = float(row[3])
                        pdb_dict[pdb_code] = log_kd_ki
            return pdb_dict

        file_path = 'INDEX_general_NL.2020'
        self.pdb_dict = read_pdb_file(file_path)

        # 打印字典内容
        # for pdb_code, log_kd_ki in pdb_dict.items():
        #     print(f"PDB code: {pdb_code}, -logKd/Ki: {log_kd_ki}")
        if not args.test:
