...
def get_nonBond_pair(m1,m2):
    ...
    rna = plf.Molecule(m1)
    # print("pair")
    fp.run_from_iterable([rna],prot,progress=False,n_jobs = 1)
    # print(fp)
    ...
    r_res_to_idx = defaultdict(dict)
    # 获取RNA和分子中的所有原子坐标
    prot_coords = m2.GetConformer().GetPositions()
    rna_coords = m1.GetConformer().GetPositions()
    # 定义pocket的半径
    pocket_radius = 8
    pocket = []
    # 遍历RNA中的每个原子
    ...
    try:
        for atom in m2.GetAtoms():
            ...
            for ligand_atom in m1.GetAtoms():
                ...
                ligand_atom_pos = rna_coords[ligand_atom_idx]
    ...
    for atom in m1.GetAtoms():
        ...
        res_name = str(plf.residue.ResidueId.from_atom(atom))
        # print(res_name,atom_idx)
        r_res_to_idx[res_name][len(r_res_to_idx[res_name])] = atom_idx
    ...
    for key in df.keys():
        ...
        # print(ligand_name.strip(),res_name.strip(),inter_type)
    #     lig_atom_num,res_atom_num = df[key][0]
        # print(df[key][ligand_name.strip()])
        # print(res_to_idx[res_name])
        lig_atom_indices = list(r_res_to_idx[ligand_name].values())
        # print(lig_atom_indices)
        # 2. 获取碱基所有原子 index
        res_atom_indices = list(res_to_idx[res_name].values())
        # print(res_atom_indices)
        # print(lig_atom_idx)
        # 3. 组合所有 pair
        for res_atom_idx in res_atom_indices:
            for lig_atom_idx in lig_atom_indices:
                nonbond_pairs.append((lig_atom_idx, res_atom_idx))
