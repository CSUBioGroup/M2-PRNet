...
class LogScaler:
    ...
    def fit(self, targets):
        ...
        if min_val <= 0:
            self.shift = -min_val*2  + self.epsilon
        else:
            self.shift = 0
            
    def transform(self, targets):
        targets_shifted = targets + self.shift
        return np.log1p(targets_shifted + 1)  # log(1+x)，更稳定 不然训练初始会爆炸
        
    def inverse_transform(self, scaled_targets):
        exp_targets = np.expm1(scaled_targets) -1  # exp(x)-1
        return exp_targets - self.shift
"""
...
class MLPReadout(nn.Module):
    ...
    def __init__(self, args): 
        ...
        self.FC = nn.ModuleList([nn.Linear(self.args.n_out_feature , self.args.d_FC_layer) if i==0 else
                                nn.Linear(self.args.d_FC_layer, 1) if i==self.args.n_FC_layer-1  else
