...
def evaluator_m(model,loader,loss_fn,args,test_sampler):
    model.eval()
    with torch.no_grad():
        test_losses,test_true,test_pred,keys = [], [],[],[]
        for i_batch, (g,full_g,Y,key,pro_feat, rna_feat, mol_indicator, chain_indicator,
                   pro_coords,rna_coords, prot_emb, rna_emb, neighbor_matrix_padded, prot_len, rna_len, mask) in enumerate(loader):
            g = g.to(args.device,non_blocking=True)
            full_g = full_g.to(args.device,non_blocking=True)
            ...
            Y = Y.unsqueeze(-1)
            print(Y)
            pred,score_matrix = model(g,full_g,pro_feat, rna_feat, mol_indicator, chain_indicator,
                   pro_coords,rna_coords, prot_emb, rna_emb, neighbor_matrix_padded, prot_len, rna_len, mask,getScore=True)
            loss = loss_fn(pred ,Y) 
...
def testAndPrint(model,loader,loss_fn,args,test_sampler):
    ...
    return test_losses,test_true,test_pred,keys
def testAndPrint_m(model,loader,loss_fn,args,test_sampler):
    model.eval()
    df = pd.read_csv('sort_results.csv')
    results_df = pd.DataFrame(columns=['key', 'pred'])
    with torch.no_grad():
        test_losses,test_true,test_pred,keys = [], [],[],[]
        for i_batch, (g,full_g,Y,key,pro_feat, rna_feat, mol_indicator, chain_indicator,
                   pro_coords,rna_coords, prot_emb, rna_emb, neighbor_matrix_padded, prot_len, rna_len, mask) in tqdm.tqdm(enumerate(loader),total = len(loader)):
            # print(key)
            g = g.to(args.device,non_blocking=True)
            full_g = full_g.to(args.device,non_blocking=True)
            pro_feat = pro_feat.to(args.device,non_blocking=True)
            ...
            Y = Y.to(args.device,non_blocking=True)
            Y = Y.unsqueeze(-1)
            pred = model(g,full_g,pro_feat, rna_feat, mol_indicator, chain_indicator,
                   pro_coords,rna_coords, prot_emb, rna_emb, neighbor_matrix_padded, prot_len, rna_len, mask)
            loss = loss_fn(pred ,Y) 
            for k, p in zip(key, pred):
                df.loc[df['Name'] == k, 'RNAEquiScore'] = p.item()
            for k, p in zip(key, Y):
...
def train_m(model,args,optimizer,loss_fn,train_dataloader,scheduler):
    # collect losses of each iteration
    train_losses = [] 
    model.train()
    # 配置示例
    # config = {
    #     'rna_biochem_dim': 9,      # RNA生化特征维度
    #     'rna_seq_dim': 1280,         # RNA序列特征维度
    #     'protein_biochem_dim': 8, # 蛋白质生化特征维度
    #     'protein_seq_dim': 1280,    # 蛋白质序列特征维度
    #     'hidden_dim': 256,
    #     'num_layers': 6,
    #     'num_heads': 8,
    #     'dropout': 0.1,
    ...
    # 创建模型
    # model2 = RNAProteinGraphTransformer(config)
    # model2 = model2.to(args.device)
    for i_batch, (g,full_g,Y,key,pro_feat, rna_feat, mol_indicator, chain_indicator,
                   pro_coords,rna_coords, prot_emb, rna_emb, neighbor_matrix_padded, prot_len, rna_len, mask) in tqdm.tqdm(enumerate(train_dataloader),total = len(train_dataloader)):
        # print(Y)
        g = g.to(args.device,non_blocking=True)
        full_g = full_g.to(args.device,non_blocking=True)
        pro_feat = pro_feat.to(args.device,non_blocking=True)
        ...
        neighbor_matrix_padded = neighbor_matrix_padded.to(args.device,non_blocking=True)
        mask = mask.to(args.device,non_blocking=True)
        # model2(rna_feat,rna_emb, rna_coords, pro_feat,prot_emb,pro_coords, mol_indicator, chain_indicator,
        ...
        logits = model(g,full_g, rna_feat, rna_emb, rna_coords, pro_feat,prot_emb,pro_coords, mol_indicator, chain_indicator,
                    neighbor_matrix_padded, mask)
        Y = Y.unsqueeze(-1)
        # print(logits)
        # print(Y.shape)
