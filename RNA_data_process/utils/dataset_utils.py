...
def atom_feature_attentive_FP(atom,
    ...
    Chem.rdchem.HybridizationType.SP3D2, Chem.rdchem.HybridizationType.S,
    Chem.rdchem.HybridizationType.UNSPECIFIED, 'other'
    ]
    ...
    if bool_id_feat:
        ...
        results = GetNum(
          ...
          ['C', 'N', 'O', 'S', 'F', 'P', 'Cl', 'Br', 'B', 'H','other']) \
              + GetNum(atom.GetDegree(),[0, 1, 2, 3, 4, 5]) + \
                  ...
                  GetNum(atom.GetHybridization(), allowed_hybridizations) + [atom.GetIsAromatic()]
        ...
        if use_chirality:
            try:
                # print(atom.HasProp('_ChiralityPossible'))
...
def preprocess_item_map(item, args, adj, n1, n2, ligand_mapping,pocket_mapping ):
    """
    ...
    'OMG': 'G', 'PSU': 'U', '5MC': 'C', 'M2G': 'G',
    'H2U': 'U', '5MU': 'U', '1MG' : 'G', 'OMC': 'C','OMU':'U',
    'A2M': 'A', 'IU' : 'U', 
    }
    ...
    # print(pocket_mapping)
    for idx, atom_info in pocket_mapping.items():
        ...
        node_bases[idx] = protein_dict[base_name]
    ...
    for idx, atom_info in ligand_mapping.items():
        node_types[idx+len(pocket_mapping)] = 2  # Example: Mark as Ligand atom (type = 2)
        # print(atom_info)
        base_name = atom_info['bases'].split('_')[0]  # 提取碱基部分，例如 "CD"
        node_id = int(atom_info['bases'].split('_')[1])  # 提取节点索引部分，例如 5
        # print(base_name)
        try:
            node_bases[idx+len(pocket_mapping)] = main_base_to_int[base_category_map.get(base_name.strip())] +20
        except KeyError:
            print(f"Warning: Base name '{base_name}' not found in mapping. Assigning 'Unknown'.")
        # print(node_bases[idx+len(pocket_mapping)] )
        # print(f"Base Name: {base_name}, Node Index: {node_id}")
        node_idx[idx+len(pocket_mapping)] = int(node_id) # Example: store base information ligand is zero
        # node_bases[idx+len(pocket_mapping)] = 5
        node_atom_idx[idx+len(pocket_mapping)] = atom_info['rdkit_index'] + len(pocket_mapping)
        # print(node_atom_idx[idx+len(pocket_mapping)])
        node_atom_type[idx+len(pocket_mapping)] = atom_info['type_id']
    # print("5")
    ...
    g.ndata['type'] = node_types_tensor  # procket :1 or ligand: 2 or virtual: 0
